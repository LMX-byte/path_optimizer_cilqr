...
class Eigen2CVBase {
 ...
 protected:
  ...
  void mapPlaneMemory(const Derived& src) {
    ...
    if (isRowMajor)
      this->mapPlaneMemoryRowMajor(src.data(), src.rows(), src.cols(), stride);
    else
      this->mapPlaneMemoryColMajor(src.data(), src.rows(), src.cols(), stride);
  }
...
class Eigen2CV<Derived, Eigen::EigenBase<Derived>, details::Const>
 ...
 public:
  typedef typename Derived::Scalar Scalar;
  typedef
      typename Eigen::internal::plain_matrix_type<Derived>::type StorageType;
