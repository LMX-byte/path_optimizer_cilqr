...
int main(int argc, char **argv) {
  ...
  while (nh.ok()) {
    ...
    if (reference_points.size() >= 2) {
      ...
      if (std::sqrt(std::pow(p1.x - p2.x, 2) + std::pow(p1.y - p2.y, 2)) <=
          0.001) {
        reference_points.clear();
        reference_rcv = false;
      }
    ...
    visualization_msgs::Marker reference_marker = markers.newSphereList(
        0.5, "reference point", id++, ros_viz_tools::RED, marker_frame_id);
    ...
    visualization_msgs::Marker start_marker =
        markers.newArrow(scale, start_pose, "start point", id++,
                         ros_viz_tools::CYAN, marker_frame_id);
    ...
    visualization_msgs::Marker end_marker =
        markers.newArrow(scale, end_pose, "end point", id++,
                         ros_viz_tools::CYAN, marker_frame_id);
    markers.append(end_marker);

    if (reference_rcv && start_state_rcv && end_state_rcv) {
      Test::Map map(grid_map);
      Test::ReferenceLineProcessor reference_line_processor(reference_points,
                                                            map, start_state);
      std::shared_ptr<PathPlanning::ReferenceLine> ref_line_ptr =
          std::make_shared<PathPlanning::ReferenceLine>();
      std::shared_ptr<PathPlanning::FreeSpace> free_space_ptr =
          std::make_shared<PathPlanning::FreeSpace>();
      reference_line_processor.solve(ref_line_ptr, free_space_ptr);

      visualization_msgs::Marker boundary_points_marker = markers.newSphereList(
          0.3, "boundary points", id++, ros_viz_tools::BLUE, marker_frame_id);
      ...
      path_problem_manager.formulate_path_problem(
          *free_space_ptr, *ref_line_ptr, start_state, end_state);
      Solver::ILQRSolver<PathPlanning::N_PATH_STATE,
                         PathPlanning::N_PATH_CONTROL>
          ilqr_solver(path_problem_manager);
      const auto solve_status = ilqr_solver.solve();
      // LOG(INFO) << "Solve status " << solve_status;

      visualization_msgs::Marker init_path_marker = markers.newLineStrip(
          0.3, "init path", id++, ros_viz_tools::YELLOW, marker_frame_id);
      ...
      visualization_msgs::Marker result_marker =
          markers.newLineStrip(FLAGS_vehicle_width, "optimized path", id++,
                               path_color, marker_frame_id);
      visualization_msgs::Marker vehicle_geometry_marker = markers.newLineList(
          0.02, "vehicle", id++, ros_viz_tools::GRAY, marker_frame_id);
      ...
      const auto result = PathProblemManager::transform_to_path_points(
          *ref_line_ptr, opt_path_raw);
