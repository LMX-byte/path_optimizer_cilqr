...
void PathProblemManager::formulate_path_problem(
    const FreeSpace& free_space, const ReferenceLine& reference_line,
    const PathPoint& init_state, const PathPoint& end_state) {
...
void PathProblemManager::calculate_init_trajectory(
    const ReferenceLine& reference_line, const PathPoint& init_state,
    const PathPoint& end_state) {
  _init_trajectory.clear();
  _knots.clear();
  _costs.clear();

  auto dynamics_by_kappa =
      [&reference_line](
          const Node<N_PATH_STATE, N_PATH_CONTROL>& node,
          double move_dist) -> Node<N_PATH_STATE, N_PATH_CONTROL> {
    const double kappa_ref =
        reference_line.get_reference_point(node.sample()).kappa;
  ...
  (*node.mutable_state())(HD_INDEX) = constrainAngle(
      init_state.theta - reference_line.get_reference_point(init_proj.s).theta);
  ...
  for (std::size_t i = 0; node.sample() + _config.delta_s < end_proj.s; ++i) {
    ...
    if (i == 0) {
      (*new_node.mutable_state())(K_INDEX) =
          init_state.dkappa * _config.delta_s + init_state.kappa;
    } else {
      // Kappa by pure pursuit.
      const auto xy = reference_line.get_xy_by_sl(
          SLPosition{new_node.sample(), new_node.state()(L_INDEX)});
      const double heading = constrainAngle(
          reference_line.get_reference_point(new_node.sample()).theta +
          new_node.state()(HD_INDEX));
      const auto pursuit_point =
          reference_line.get_reference_point(new_node.sample() + pursuit_dist);
      const double dist = distance(xy, pursuit_point);
      double kappa =
          std::fabs(dist) > 1.0
              ? 2 *
                    sin(atan2(pursuit_point.y - xy.y, pursuit_point.x - xy.x) -
                        heading) /
                    dist
              : 0.0;
  ...
  for (std::size_t i = 0; i < _init_trajectory.size() - 1; ++i) {
    const double delta_s =
        _config.delta_s *
        (1 - reference_line.get_reference_point(_init_trajectory.at(i).sample())
                 .kappa) /
        cos(_init_trajectory.at(i).state()(HD_INDEX));
    (*_init_trajectory.at(i).mutable_control())(KR_INDEX) =
        (_init_trajectory.at(i + 1).state()(K_INDEX) -
         _init_trajectory.at(i).state()(K_INDEX)) /
        delta_s;
  }
}
...
std::vector<PathPoint> PathProblemManager::transform_to_path_points(
  ...
  for (const auto& pt : trajectory) {
    ...
    path_point.x = xy.x;
    path_point.y = xy.y;
    path_point.theta = constrainAngle(
        reference_line.get_reference_point(sl.s).theta + pt.state()(HD_INDEX));
...
void PathProblemManager::add_costs(const FreeSpace& free_space,
  ...
  std::shared_ptr<PathCost> kappa_rate_cost_ptr(
      new KappaRateCost(WEIGHT_KAPPA_RATE));
  std::shared_ptr<PathCost> rear_boundary_constraint_ptr(
      new RearBoundaryConstraint(free_space, 0.5, 2.5));
  std::shared_ptr<PathCost> front_boundary_constraint_ptr(
      new FrontBoundaryConstraint(free_space, 0.5, 2.5));
  ...
  const double end_state_heading_diff =
      constrainAngle(end_state.theta -
                     reference_line.get_reference_point(_knots.back()).theta);
  std::shared_ptr<PathCost> end_state_cost_ptr(
      new TargetStateCost(end_state_l, end_state_heading_diff, WEIGHT_END_L,
                          WEIGHT_END_HEADING_DIFF, "end_state"));
  ...
  for (std::size_t step = 0; step < num_steps(); ++step) {
    ...
    if (step > 0) {
      _costs.at(step)[rear_boundary_constraint_ptr->name()] =
          rear_boundary_constraint_ptr;
      _costs.at(step)[front_boundary_constraint_ptr->name()] =
          front_boundary_constraint_ptr;
...
Variable<N_PATH_STATE> FrenetPathDynamics::move_forward(
    const Node<N_PATH_STATE, N_PATH_CONTROL>& node, double move_dist) const {
  const double kappa_ref =
      _reference_line.get_reference_point(node.sample()).kappa;
  const double l = node.state()(L_INDEX);
  const double hd = node.state()(HD_INDEX);
  const double kappa = node.state()(K_INDEX);
  ...
  ret << l + dl * move_dist, constrainAngle(hd + dhd * move_dist),
      kappa + dk * move_dist;
  return ret;
}
...
Eigen::Matrix<double, N_PATH_STATE, N_PATH_STATE> FrenetPathDynamics::dx(
  ...
  ret << 1 - move_dist * tan(hd) * kappa_ref,
      move_dist * (1 - kappa_ref * l) / cos(hd) / cos(hd), 0.0,
      -move_dist * kappa * kappa_ref / cos(hd),
      1 + move_dist * (1 - kappa_ref * l) * kappa * tan(hd) / cos(hd),
      move_dist * (1 - kappa_ref * l) / cos(hd),
      -move_dist * kappa_rate * kappa_ref / cos(hd),
      move_dist * (1 - kappa_ref * l) * kappa_rate * tan(hd) / cos(hd), 1.0;
  return ret;
}
