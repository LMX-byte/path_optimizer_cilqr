...
class ILQRSolver {
 public:
  ILQRSolver(ProblemManager<N_STATE, N_CONTROL>& problem_manager)
    ...
    _current_cost =
        problem_manager.calculate_total_cost(problem_manager.init_trajectory());
  };
  ILQRSolveStatus solve();
  Trajectory<N_STATE, N_CONTROL> final_trajectory() const {
...
ILQRSolveStatus ILQRSolver<N_STATE, N_CONTROL>::solve() {
  if (not _problem_manager.problem_formulated() ||
      _current_trajectory.empty()) {
  ...
  for (std::size_t i = 0; i < _num_steps; ++i) {
    ...
    if (i < _num_steps - 1) {
      const double move_dist =
          _problem_manager.knots().at(i + 1) - _problem_manager.knots().at(i);
      derivative.fx =
          _problem_manager.dynamics().dx(_current_trajectory.at(i), move_dist);
      derivative.fu =
          _problem_manager.dynamics().du(_current_trajectory.at(i), move_dist);
    }
    // LOG_IF(INFO, _iter == 0 && i < 5) << "[Test] i " << i << ", lx " <<
    // derivative.lx << " lu " << derivative.lu << " lxx " << derivative.lxx;
  }
}
...
void ILQRSolver<N_STATE, N_CONTROL>::backward_pass() {
  ...
  for (int i = _num_steps - 2; i >= 0; --i) {
    ...
    const auto qxx =
        derivative.lxx + derivative.fx.transpose() * _vxx * derivative.fx;
    const auto quu =
        derivative.luu + derivative.fu.transpose() * _vxx * derivative.fu +
        _mu * Eigen::Matrix<double, N_CONTROL, N_CONTROL>::Identity();
    const auto qux =
        derivative.lux + derivative.fu.transpose() * _vxx * derivative.fx;

    Eigen::LLT<Eigen::MatrixXd> llt_quu(quu);
    if (llt_quu.info() == Eigen::NumericalIssue) {
      LOG(INFO) << "[Backward pass] Non-PD quu at index " << i << ", mu "
                << _mu;
    ...
    _vx = qx + _K.at(i).transpose() * quu * _k.at(i) +
          _K.at(i).transpose() * qu + qux.transpose() * _k.at(i);
    _vxx = qxx + _K.at(i).transpose() * quu * _K.at(i) +
           _K.at(i).transpose() * qux + qux.transpose() * _K.at(i);

    if (i < _num_steps - 1) {
      _approx_cost_decay_info.first += _k.at(i).transpose() * qu;
      _approx_cost_decay_info.second +=
          (0.5 * _k.at(i).transpose() * quu * _k.at(i))(0);
    }
  }
...
void ILQRSolver<N_STATE, N_CONTROL>::forward_pass() {
  ...
  double alpha = 1.0;
  double new_cost = 0.0;
  LOG(INFO) << "[Forward pass] Iter " << _iter << ", current cost "
            << _current_cost;
  while (alpha > _ilqr_config.min_alpha) {
    ...
    for (std::size_t i = 0; i < _num_steps - 1; ++i) {
      *(new_trajectory.at(i).mutable_control()) =
          _current_trajectory.at(i).control() + alpha * _k.at(i) +
          _K.at(i) * (new_trajectory.at(i).state() -
                      _current_trajectory.at(i).state());
      *(new_trajectory.at(i + 1).mutable_state()) =
          _problem_manager.dynamics().move_forward(
              new_trajectory.at(i), _problem_manager.knots().at(i + 1) -
                                        _problem_manager.knots().at(i));
    }
    ...
    //         LOG(INFO) << i << " old state " <<
    //         _current_trajectory.at(i).state() << ", control " <<
    //         _current_trajectory.at(i).control(); LOG(INFO) << i << " new
    //         state " << new_trajectory.at(i).state() << ", control " <<
    //         new_trajectory.at(i).control();
    //     }
    // }
    if (std::fabs(alpha - 1.0) < EPS &&
        std::fabs(actual_cost_decay) < _ilqr_config.convergence_threshold) {
      LOG(INFO) << "[Forward pass] Iter " << _iter
                << ", optimization has converged.";
      _current_solve_status = LQRSolveStatus::CONVERGED;
      return;
    }
    const double approx_cost_decay =
        -(alpha * _approx_cost_decay_info.first +
          alpha * alpha * _approx_cost_decay_info.second);
    LOG(INFO) << "[Forward pass] Iter " << _iter << ", alpha " << alpha
              << ", actual cost decay " << actual_cost_decay << ", approx "
              << approx_cost_decay;
    if (actual_cost_decay > 0.0 &&
        (approx_cost_decay < 0.0 || actual_cost_decay / approx_cost_decay >
                                        _ilqr_config.accept_step_threshold)) {
      // if (true) {
      LOG(INFO) << "[Forward pass] Iter " << _iter << ", accept alpha " << alpha
                << ", cost " << new_cost;
